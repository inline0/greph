...
Build a fresh trigram + identifier postings index for `$rootPath`. The index is stored at `$indexPath` (defaults to `$rootPath/.greph-index`). `$lifecycle` can be `static`, `manual-refresh`, `opportunistic-refresh`, or `strict-stale-check`. Returns an `IndexBuildResult` with the file count, trigram count, and added/updated/deleted/unchanged counts.
...
Run warmed text search across several text index directories. Greph searches each applicable index, de-duplicates by absolute file path, and falls back to native scan only for paths outside every provided index root.
...
Load an index-set manifest, defaulting to `.greph-index-set.json` in the current working directory.
...
Resolve the selected `text` entries from an index-set manifest and run the equivalent of `searchTextIndexedMany`.
...
Run warmed AST fact search across several AST index directories.
...
Resolve the selected `ast-index` entries from an index-set manifest and run the equivalent of `searchAstIndexedMany`.
...
Run warmed cached-AST search across several cache directories.
...
Resolve the selected `ast-cache` entries from an index-set manifest and run the equivalent of `searchAstCachedMany`.
...
`tracePlan` is consumed by warmed indexed searchers and `greph-index` CLI flows to emit planner diagnostics to `stderr`. Native `searchText` itself does not use it.
...
The default file type filter for AST mode is `php`. When `skipParseErrors` is `true` (default), files that fail to parse are silently skipped; flip it to `false` to surface parse errors as exceptions. `tracePlan` is consumed by warmed AST searchers and `greph-index ast-index|ast-cache search` to emit candidate-pruning diagnostics.
