...
It also ships warmed indexed modes for both text and AST workloads, lifecycle-aware refresh policies, manifest-backed warmed index sets, and `rg` / `sg` compatibility wrappers for tool and agent environments.
...
- Lifecycle-aware warmed indexes: `static`, `manual-refresh`, `opportunistic-refresh`, `strict-stale-check`
- Multi-index and named set search across warmed text, AST fact, and AST cache stores
- Planner diagnostics with `--trace-plan` and index-origin labeling for warmed searches
