...
Indexes live in `.greph-index`, `.greph-ast-index`, and `.greph-ast-cache` at the project root by default.
...
- keep a stable baseline as `static`
- keep actively changing overlays as `opportunistic-refresh`
- inspect stale behavior with `stats --dry-refresh`
...
See [Indexed / Text](/docs/indexed/text) and [Indexed / AST](/docs/indexed/ast) for the full warmed workflow.
